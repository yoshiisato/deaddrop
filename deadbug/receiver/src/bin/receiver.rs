...
use rust_omr::types::{decode_payloads, decode_pk_detect_from_hex, encode_payloads, encode_pk_detect_to_hex, OMRItem, Payload};
...
pub fn main() {
    let mut receiver = Receiver::new();
        ...
        let decoded_detection_key = decode_pk_detect_from_hex(&det_key_enc)
            .expect("Failed to decode detection key from hex");
        ...
        let digest = rust_omr::detector::detect(&receiver.public_params, &bulletin_board, decoded_detection_key.as_slice(), 1);
        ...
        println!("Decrypted Bug Report: {:?}", String::from_utf8(decrypted_report).unwrap_or_else(|_| "Failed to convert decrypted report to string".to_string()));
